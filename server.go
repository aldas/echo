...
func (sc *StartConfig) Start(e *Echo) error {
	...
	server := http.Server{
		Handler:  e,
		ErrorLog: log.New(logger.Writer(), "echo: ", 0),
	}
...
func (sc *StartConfig) StartH2CServer(e *Echo, h2s *http2.Server) error {
	logger := e.Logger
	server := http.Server{
...
func serve(sc *StartConfig, server *http.Server, listener net.Listener, logger Logger) error {
	...
	if sc.GracefulShutdown.Context != nil {
		config := sc.GracefulShutdown
		gsCtx, cancel := stdContext.WithCancel(config.Context)
		config.Context = gsCtx
		defer cancel() // make sure this graceful coroutine will end when serve returns by some other means
		go gracefulShutdown(server, config, logger)
	}
...
func gracefulShutdown(server *http.Server, config GracefulShutdownConfig, logger Logger) {
