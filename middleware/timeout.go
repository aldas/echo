...
func Timeout() echo.MiddlewareFunc {
	return MustTimeoutWithConfig(DefaultTimeoutConfig)
}

// MustTimeoutWithConfig returns a Timeout middleware with config or panics on invalid configuration.
func MustTimeoutWithConfig(config TimeoutConfig) echo.MiddlewareFunc {
	mw, err := TimeoutWithConfig(config)
	if err != nil {
		panic(err)
	}
	return mw
...
// See: `Timeout()`.
func TimeoutWithConfig(config TimeoutConfig) (echo.MiddlewareFunc, error) {
	// Defaults
