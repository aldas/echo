...
func TestMustBasicAuth_panic(t *testing.T) {
	assert.Panics(t, func() {
		mw := MustBasicAuthWithConfig(BasicAuthConfig{Validator: nil})
		assert.NotNil(t, mw)
	})

	mw := MustBasicAuthWithConfig(BasicAuthConfig{Validator: func(user string, password string, c echo.Context) (bool, error) {
		return true, nil
	}})
	assert.NotNil(t, mw)
}
