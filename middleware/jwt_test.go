...
func createTestParseTokenFuncForJWTGo(signingMethod string, signingKey interface{}) func(auth string, c echo.Context) (interface{}, error) {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expPanic {
				assert.Panics(t, func() {
					MustJWTWithConfig(tc.config)
				}, tc.name)
				return
			}

			if tc.expErrCode != 0 {
				h := MustJWTWithConfig(tc.config)(handler)
...
func TestJWTConfig_skipper(t *testing.T) {
	...
	e.Use(MustJWTWithConfig(JWTConfig{
...
func TestJWTConfig_extractorErrorHandling(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			e.Use(MustJWTWithConfig(tc.given))
...
func TestJWTConfig_parseTokenErrorHandling(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			e.Use(MustJWTWithConfig(config))
...
func TestJWTWithConfig_CallNextOnNilErrorHandlerResult(t *testing.T) {
	var testCases = []struct {
	...
	}{
		{
			...
			givenCallNext: true,
			givenErrorHandler: func(err error, c echo.Context) error {
				return nil
			},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			mw, err := JWTWithConfig(
				JWTConfig{
					...
					CallNextOnNilErrorHandlerResult: tc.givenCallNext,
				},
			)
			assert.NoError(t, err)
			e.Use(mw)
