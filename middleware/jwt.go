...
type JWTConfig struct {
	...
	// ErrorHandler defines a function which is executed for missing (ErrExtractionValueMissing) or an invalid token.
	// It may be used to define a custom JWT error.
	ErrorHandler JWTErrorHandlerWithContext

	// CallNextOnNilErrorHandlerResult if true makes middleware to call next(c) when ErrorHandler returns no error.
	...
	CallNextOnNilErrorHandlerResult bool
...
// MustJWTWithConfig returns a JSON Web Token (JWT) auth middleware or panics if configuration is invalid.
//
...
func MustJWTWithConfig(config JWTConfig) echo.MiddlewareFunc {
	mw, err := JWTWithConfig(config)
	if err != nil {
		panic(err)
	}
	return mw
...
// See: https://jwt.io/introduction
// See `JWTConfig.TokenLookup`
func JWTWithConfig(config JWTConfig) (echo.MiddlewareFunc, error) {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if config.ErrorHandler != nil {
				...
				handledErr := config.ErrorHandler(err, c)
				if config.CallNextOnNilErrorHandlerResult && handledErr == nil {
					return next(c)
				}
				return handledErr
			}
