...
// MustStaticWithConfig returns a Static middleware to serves static content or panics on invalid configuration.
func MustStaticWithConfig(config StaticConfig) echo.MiddlewareFunc {
	mw, err := StaticWithConfig(config)
	if err != nil {
		panic(err)
	}
	return mw
...
func StaticWithConfig(config StaticConfig) (echo.MiddlewareFunc, error) {
	// Defaults
	if config.Root == "" {
