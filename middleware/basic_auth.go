...
// MustBasicAuthWithConfig returns an BasicAuthWithConfig middleware or panics on error.
func MustBasicAuthWithConfig(config BasicAuthConfig) echo.MiddlewareFunc {
	mw, err := BasicAuthWithConfig(config)
	if err != nil {
		panic(err)
	}
	return mw
...
func BasicAuthWithConfig(config BasicAuthConfig) (echo.MiddlewareFunc, error) {
