...
func TestBodyLimit(t *testing.T) {
	...
	mw, err := BodyLimitWithConfig(BodyLimitConfig{LimitBytes: 2 * MB})
	...
	mw, err = BodyLimitWithConfig(BodyLimitConfig{LimitBytes: 2})
	assert.NoError(t, err)
...
func TestBodyLimit_skipper(t *testing.T) {
	...
	mw, err := BodyLimitWithConfig(BodyLimitConfig{
...
func TestMustBodyLimit(t *testing.T) {
	e := echo.New()
	hw := []byte("Hello, World!")
	req := httptest.NewRequest(http.MethodPost, "/", bytes.NewReader(hw))
	rec := httptest.NewRecorder()
	...
	mw := MustBodyLimitWithConfig(BodyLimitConfig{LimitBytes: 2 * MB})
