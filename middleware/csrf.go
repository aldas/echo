...
func CSRF() echo.MiddlewareFunc {
	c := DefaultCSRFConfig
	return MustCSRFWithConfig(c)
}

// MustCSRFWithConfig returns a CSRF middleware with config or panics on invalid configuration.
func MustCSRFWithConfig(config CSRFConfig) echo.MiddlewareFunc {
	mw, err := CSRFWithConfig(config)
	if err != nil {
		panic(err)
	}
	return mw
...
// See `CSRF()`.
func CSRFWithConfig(config CSRFConfig) (echo.MiddlewareFunc, error) {
	// Defaults
